        ...
        if(typeof elo == 'number') {
            ...
            if(limitStrength) {
                ...
                if(didUserUpdateSetting)
                    toast.message(`Engine skill level ${skillLevel}, search depth ${depth}`, 30000);
            } else {
                this.setEngineLimitStrength(false, profile);

                this.setEngineSkillLevel(30, profile);
                ...
                if(elo !== 3200) {
                    ...
                    if(didUserUpdateSetting)
                        toast.message(`Engine not limited in strength, search depth ${depth}`, 30000);
                } else {
                    this.pV[profile].searchDepth = null;

                    if(didUserUpdateSetting)
                        toast.message(`Engine has no strength limitations, running infinite depth!`, 30000);
                }
            }
        }
    }
